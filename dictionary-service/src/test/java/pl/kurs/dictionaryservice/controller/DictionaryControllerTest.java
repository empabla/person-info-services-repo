...
import pl.kurs.dictionaryservice.DictionaryServiceApplication;
import pl.kurs.dictionaryservice.command.CreateDictionaryCommand;
import pl.kurs.dictionaryservice.command.UpdateDictionaryCommand;
import pl.kurs.dictionaryservice.model.Dictionary;
import pl.kurs.dictionaryservice.model.DictionaryValue;
import pl.kurs.dictionaryservice.repository.DictionaryRepository;
...
class DictionaryControllerTest {
    ...
    public void shouldGetAllDictionaries() throws Exception {
        ...
        //when
        ResultActions resultActions = mockMvc.perform(MockMvcRequestBuilders.get("/api/dictionaries"));
        //then
        resultActions
                .andExpect(jsonPath("$[0].id").value(notNullValue()))
                .andExpect(jsonPath("$[0].name").value("type"))
                .andExpect(jsonPath("$[1].id").value(notNullValue()))
                .andExpect(jsonPath("$[1].name").value("position"))
                .andExpect(status().isOk());
    }
    ...
    public void shouldUpdateDictionaryName() throws Exception {
        ...
        //when
        ResultActions resultActions = mockMvc.perform(MockMvcRequestBuilders.put("/api/dictionaries")
                .contentType(MediaType.APPLICATION_JSON)
                .content(jsonForTest));
        //then
        resultActions
